...
font-family:"Times New Roman","serif"'>STMIK AMIKOM Purwokerto</span></b></p>
...
<div class="pull-right">
   <div class="empty">
		   ...
		   <script type="text/javascript">
            ...
            normal'><span style='font-size:12.0pt;mso-bidi-font-size:11.0pt;font-family:
           "Times New Roman","serif"'>
			<a id="return" data-placement="top" class="btn btn-success" title="Click to Return" href="dashboard.php?module=data_stock_barang"><i class="icon-arrow-left"></i> Back</a></p>		
			<script type="text/javascript">
			$(document).ready(function(){
			$('#return').tooltip('show');
			$('#return').tooltip('hide');
...
<?php
		$device_query = mysql_query("select * from stdevice, device_name,location_details where stdevice.dev_id=device_name.dev_id and stdevice.id=location_details.id and location_details.stdev_id='$_GET[stdev_id]'  order by stdevice.id DESC ")or die(mysql_error());
...
?>
 <tr style='mso-yfti-irow:1'>
  ...
  mso-border-alt:solid windowtext .5pt;padding:0in 5.4pt 0in 5.4pt'>
  <p class=MsoNormal style='margin-bottom:0in;margin-bottom:.0001pt;line-height:
  normal'><span style='font-family:"Times New Roman","serif"'><?php echo $row['jumlah']; ?><o:p></o:p></span></p>
  </td>
...
<table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0
 ...
 <tr style='mso-yfti-irow:0;mso-yfti-firstrow:yes'>
  ...
  style='font-size:14.0pt;mso-bidi-font-size:11.0pt;font-family:"Times New Roman","serif"'>REKAP DATA</span></b><b
...
$new = mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id JOIN location_details ON stdevice.id=location_details.id
			   where dev_status = 'New' and stdev_id='$_GET[stdev_id]'")or die(mysql_error());
...
$new = mysql_query("select * from stdevice 
         LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id JOIN location_details ON stdevice.id=location_details.id
         where dev_status = 'Used' and stdev_id='$_GET[stdev_id]'")or die(mysql_error());
$count = mysql_num_rows($new);		                                               
?>  
  <td width=171 valign=top style='width:128.35pt;border-top:none;border-left:
  none;border-bottom:solid windowtext 1.0pt;border-right:solid windowtext 1.0pt;
...
$new = mysql_query("select * from stdevice 
         LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id JOIN location_details ON stdevice.id=location_details.id
         where dev_status = 'Repaired' and stdev_id='$_GET[stdev_id]'")or die(mysql_error());
$count = mysql_num_rows($new);		                                               
?> 
...
$new = mysql_query("select * from stdevice 
         LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id JOIN location_details ON stdevice.id=location_details.id
         where dev_status = 'Damaged' and stdev_id='$_GET[stdev_id]'")or die(mysql_error());
$count = mysql_num_rows($new);		                                               
?> 
...
<hr>
		<footer>
           <p>All Right Reserved System Powered by STMIK AMIKOM Purwokerto 2016</p>
        <footer>
</center>
</div>
</html>